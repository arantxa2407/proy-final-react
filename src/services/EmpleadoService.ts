...
createEmpleado(empleado: Omit<Empleado, 'id'> & { id: number }) {
    const userData = {
        ...
        edad: empleado.edad,
        roles: [{ id: parseInt(empleado.id.toString()) }]
    };
    return axios.post<Empleado>(API_URL, userData);
}

updateEmpleado(id: number, empleado: Partial<Empleado> & { roleId?: string }) {
    const userData: {
      ...
      nombre: empleado.nombre,
      apellido: empleado.apellido,
      rol: [{ id: parseInt(empleado.roleId || '1') }],
      correo: empleado.correo,
      telefono: empleado.telefono,
      direccion: empleado.direccion,
      turno: empleado.turno,
